...
import com.demo.S502.Repository.IRoundRepository;
...
public class PlayServiceImpl implements DicePlayService {
    ...
    //Mètode que elimina la partida amb l'id indicat 
	@Override
	public void deleteRoundsById(int id) {
		roundy.deleteById(id);
	...
	public void deleteAllRoundsByPlayerId(int id) {
		PlayerResponseDto player = findById(id);
		List <RoundResponseDto> rounds = player.getRounds();
		rounds.forEach(round -> roundy.delete(round));
